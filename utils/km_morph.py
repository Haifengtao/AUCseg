...
class based_kmeans():
    ...
    def Kmeans_segment(self, ):
        # segment full tumor
        X = np.reshape(self.flair, (-1, 1))
        X = (X-X.mean())/X.std()
        model = KMeans(n_clusters=5)
        model.fit(X)
        full_label = np.zeros(X.shape)
        full_label[model.labels_ == np.argmax(model.cluster_centers_)] = 1
        ...
        X2 = np.reshape(self.t1ce, (-1, 1))
        X2 = (X2 - X2.mean()) / X2.std()
        model2 = KMeans(n_clusters=3)
        model2.fit(X2)
        enhance_tumor = np.zeros(X2.shape)
        enhance_tumor[model2.labels_ == np.argmax(model2.cluster_centers_)] = 1
...
if __name__ == '__main__':
    ...
    print("running clusters 5")
    print(len(label_path),len(t1ce_path),len(flair_path))
    allresult = {}
    for i in range(0,len(label_path)):
        print(label_path[i])
        ...
        target_path = label_path[i][:-7] + "seg_km_5.nii"
    ...
    with open("./result_km_5.pkl", "wb") as f:
